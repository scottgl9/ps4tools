...
#define PS4_PKG_ENTRY_TYPE_DIGEST_TABLE  0x0001
#define PS4_PKG_ENTRY_TYPE_0x800         0x0010
#define PS4_PKG_ENTRY_TYPE_0x200         0x0020
#define PS4_PKG_ENTRY_TYPE_0x180         0x0080
...
int main (int argc, char *argv[])
{
	...
	for(i = 0; i < bswap_16(m_header.table_entries_num); i++)
	{
			...
			|| (((bswap_32(entries[i].type) & PS4_PKG_ENTRY_TYPE_FILE2) == PS4_PKG_ENTRY_TYPE_FILE2)))
		{
			...
			if(entry_files[i].name == NULL) entry_files[i].name = file_name_list[file_count];
			file_count++;
		} 
		else 
		{
	...
	for(i = 0; i < bswap_16(m_header.table_entries_num); i++)
	{
		...
		if((bswap_32(entries[i].type) == PS4_PKG_ENTRY_TYPE_0x800)
			...
			|| (bswap_32(entries[i].type) == PS4_PKG_ENTRY_TYPE_META_TABLE))
		{
			if ((bswap_32(entries[i].type) == PS4_PKG_ENTRY_TYPE_META_TABLE)) entry_size = bswap_16(m_header.system_entries_num) * 0x20;
			else entry_size = bswap_32(entries[i].size);
			entry_offset = bswap_32(entries[i].offset);
			entry_digests = (unsigned char *)realloc(NULL, entry_size);
